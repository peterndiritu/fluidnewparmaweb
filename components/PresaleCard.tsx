...
import { Coins, Zap, ShieldCheck, Timer, ChevronDown, TrendingUp, ArrowRight, Loader2, Globe, AlertCircle, DollarSign, Gift, Briefcase, ChevronRight, Copy, Check, Info } from 'lucide-react';
...
const PresaleCard: React.FC = () => {
  ...
  // Fixed Fluid Price: 1 FLD = 1 USD
  const fldUsdPrice = 1.00;
  ...
  const handleBuy = async () => {
    ...
    sendTx(tx, {
      onSuccess: () => alert(`Purchase successful! Your ${tokenSymbol} tokens have been allocated.`),
      onError: (err) => {
  ...
  const isDataLoading = isLoadingSold || isLoadingCap;
    ...
    <div className="w-full max-w-lg bg-slate-900 border border-white/5 rounded-[3rem] p-8 md:p-10 shadow-2xl relative overflow-hidden animate-fade-in-up">
      ...
      <div className="relative z-10">
        ...
        <div className="mb-6 bg-black/30 border border-white/5 rounded-2xl p-4 flex flex-col gap-2">
          <span className="text-[7px] font-nebula font-black text-slate-500 uppercase tracking-[0.2em]">Official {tokenName} Contract</span>
        ...
        {account && (
          <div className="space-y-3 mb-6">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="p-4 bg-purple-500/10 border border-purple-500/20 rounded-[2rem] flex flex-col gap-1 transition-all hover:bg-purple-500/15">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-[7px] font-nebula font-black text-slate-400 uppercase tracking-widest">50% Airdrop</span>
                </div>
                <div className="text-white font-nebula font-black text-xs">
            ...
            <div className="p-4 bg-emerald-500/5 border border-emerald-500/10 rounded-[2rem] flex justify-between items-center px-6">
              <span className="text-[8px] font-nebula font-black text-slate-500 uppercase tracking-[0.2em]">Total Claimable</span>
        ...
        <div className="mb-6 p-4 bg-blue-500/5 border border-blue-500/10 rounded-2xl flex gap-3 items-start relative overflow-hidden group">
          ...
          <p className="text-[9px] font-bold text-slate-400 leading-relaxed uppercase tracking-tight relative z-10">
            ...
            Both purchased and airdrop tokens are <span className="text-white">claimable with the presale</span>.
          </p>
        </div>
                ...
                {showNetworkSelector && (
                    <div className="absolute left-0 right-0 top-12 bg-slate-800 border border-white/10 rounded-2xl shadow-2xl z-[60] p-1 animate-fade-in-up max-h-60 overflow-y-auto custom-scrollbar">
                        {SUPPORTED_NETWORKS.map((net) => (
                            <button key={net.id} onClick={() => handleNetworkSelect(net)} className="w-full flex items-center gap-3 p-2.5 hover:bg-white/5 rounded-xl transition-all group">
                ...
                {showTokenSelector && (
                    <div className="absolute left-0 right-0 top-12 bg-slate-800 border border-white/10 rounded-2xl shadow-2xl z-[60] p-1 animate-fade-in-up">
                        {selectedNetwork.tokens.map((token) => (
                            <button key={token.symbol} onClick={() => handleTokenSelect(token)} className="w-full flex items-center gap-3 p-2.5 hover:bg-white/5 rounded-xl transition-all group">
        ...
        <div className="space-y-3 mb-6">
          <div className="bg-black/20 border border-white/10 rounded-[2rem] p-6 focus-within:border-indigo-500/50 transition-colors relative">
            ...
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-2 w-full">
                ...
                <input type="number" value={usdAmount} onChange={(e) => handleUsdChange(e.target.value)} className="bg-transparent text-xl font-nebula font-black text-white outline-none w-full placeholder:text-slate-800" placeholder="0.0" />
              </div>
            </div>
          </div>
          ...
          <div className="bg-black/20 border border-white/10 rounded-[2rem] p-6 focus-within:border-indigo-500/50 transition-colors">
            <div className="flex justify-between mb-2">
              ...
              <span className="text-[8px] font-nebula font-black text-indigo-400 uppercase tracking-widest">Price: $1.00 USD</span>
            </div>
            <div className="flex justify-between items-center">
              <input type="number" value={fldAmount} onChange={(e) => handleFldChange(e.target.value)} className="bg-transparent text-xl font-nebula font-black text-white outline-none w-1/2 placeholder:text-slate-800" placeholder="0.0" />
        ...
        <div className="mb-6">
          ...
          <div className="flex justify-between mt-2">
             ...
             <span className="text-[7px] font-nebula font-black text-slate-500 uppercase tracking-widest">
                Hard Cap: {isLoadingCap ? '...' : cap.toLocaleString()} {tokenSymbol}
             </span>
          </div>
        </div>

        {!account ? (
          <ConnectButton client={client} wallets={wallets} theme="dark" chains={ALL_CHAINS} connectButton={{ label: "Secure Allocation", className: "!w-full !py-4 !bg-white !text-slate-950 !rounded-[1.5rem] !font-nebula !font-black !text-[10px] !uppercase !tracking-[0.3em] !shadow-2xl hover:!scale-105 active:!scale-95 !transition-all !flex !items-center !justify-center !gap-2" }} />
        ) : (
          <button onClick={handleBuy} disabled={isBuying} className="w-full py-4 bg-white text-slate-950 rounded-[1.5rem] font-nebula font-black text-[10px] uppercase tracking-[0.3em] shadow-2xl hover:scale-105 active:scale-95 disabled:opacity-50 disabled:scale-100 transition-all flex items-center justify-center gap-2">
            {isBuying ? <>Processing <Loader2 size={12} className="animate-spin" /></> : <>Buy {tokenName} <ArrowRight size={12} /></>}
          </button>
        )}
