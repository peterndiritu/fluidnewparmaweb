           ...
           {menuItems.map((item) => (
             <div key={item.id} className="group">
                <button 
                  ...
                  className={`w-full flex items-center justify-between px-4 py-3 rounded-xl transition-all duration-300 ${openDropdown === item.id ? 'bg-blue-600/10 text-blue-500 border border-blue-600/20' : 'text-slate-400 hover:bg-white/5 hover:text-white border border-transparent'}`}
                >
                ...
                <div className={`overflow-hidden transition-all duration-300 ease-in-out ${openDropdown === item.id ? 'max-h-96 opacity-100 mt-1' : 'max-h-0 opacity-0 pointer-events-none'}`}>
                   <div className="pl-11 pr-4 py-2 space-y-2">
                      {item.children.map((child) => (
                        <button 
                          key={child}
                          className="block text-[10px] font-bold text-slate-500 hover:text-blue-400 transition-colors uppercase tracking-widest text-left w-full"
